...
/**
 * @description Kue based job queue(consumer and workers pool) for sails.
 ...
 * @return {Object} sails-hook-queue which follow installable sails-hook spec
 */
module.exports = function(sails) {
    ...
    return {
        ...
        initialize: function(done) {
            ...
            // load `sails-hook-queue`
            var eventsToWaitFor = [];

            // If the hook has been deactivated, just return
            if (!config.active) {
                sails.log.info('sails-hooks-queue deactivated.');
                return done();
            }
            ...
            sails
                .after(eventsToWaitFor, function() {
                    ...
                    sails.on('lifted', function() {
                        sails.log('sails-hook-queue loaded successfully');
                    });
        ...
        reload: function(done) {
            sails.log.info('Reloading sails-hook-queues.');
            done = done || function(){};
            //get extended config
            var config = sails.config[this.configKey];
